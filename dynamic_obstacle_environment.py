...
def obstacle1(time_now, time_step):
    obs_ref = np.array([0, 0, 2], dtype=float)
    dic = {}
    obsCenter = np.array([obs_ref[0] + 8 * np.cos(0.5 * time_now),
                          obs_ref[1] + 8 * np.sin(0.5 * time_now),
                          obs_ref[2]], dtype=float)
    vObs = np.array([-4 * np.sin(0.5 * time_now), 4 * np.cos(0.5 * time_now), 0])

    time_now += time_step

    obsCenterNext = np.array([obs_ref[0] + 8 * np.cos(0.5 * time_now),
                              obs_ref[1] + 8 * np.sin(0.5 * time_now),
                              obs_ref[2]], dtype=float)
    vObsNext = np.array([-4 * np.sin(0.5 * time_now), 4 * np.cos(0.5 * time_now), 0])
    dic['v'] = vObs
...
def obstacle2(time_now, time_step):
    obs_ref = np.array([0, 0, 2], dtype=float)
    dic = {}
    obsCenter = np.array([obs_ref[0] + 6 * np.cos(time_now),
                          obs_ref[1],
                          obs_ref[2]], dtype=float)
    vObs = np.array([-6 * np.sin(time_now), 0, 0])
    time_now += time_step
    obsCenterNext = np.array([obs_ref[0] + 6 * np.cos(time_now),
                              obs_ref[1],
                              obs_ref[2]], dtype=float)
    vObsNext = np.array([-6 * np.sin(time_now), 0, 0])
...
def obstacle3(time_now, time_step):
    obs_ref = np.array([0, 0, 2], dtype=float)
    dic = {}
    obsCenter = np.array([obs_ref[0] - 10 * np.cos(0.3 * time_now),
                          obs_ref[1] - 10 * np.sin(0.3 * time_now),
                          obs_ref[2]], dtype=float)
    vObs = np.array([3 * np.sin(0.3 * time_now), -3 * np.cos(0.3 * time_now), 0])

    time_now += time_step

    obsCenterNext = np.array([obs_ref[0] - 10 * np.cos(0.3 * time_now),
                              obs_ref[1] - 10 * np.sin(0.3 * time_now),
                              obs_ref[2]], dtype=float)
    vObsNext = np.array([3 * np.sin(0.3 * time_now), -3 * np.cos(0.3 * time_now), 0])
    dic['v'] = vObs
    dic['obsCenter'] = obsCenter
    dic['vNext'] = vObsNext
    dic['obsCenterNext'] = obsCenterNext
