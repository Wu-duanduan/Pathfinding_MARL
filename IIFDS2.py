...
class IIFDS:
    ...
    def calRepulsiveMatrix(self, uavPos, obsCenter, cylinderR, row0, goal):
        ...
        max_exponent_magnitude = 100.0  # 限制指数的最大绝对值
        ...
        # 【关键修复】直接限制指数 1/row 的大小
        exponent = 1 / row
        exponent_clipped = np.clip(exponent, -max_exponent_magnitude, max_exponent_magnitude)
        # --- 修复结束 ---

        T = self.calculateT2(obsCenter, uavPos, cylinderR)

        # 使用裁剪后的指数
        denominator1 = T ** exponent_clipped + epsilon
